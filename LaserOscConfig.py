...
# change these
scan_speed = 10 # in nm/s
wth_i = 1450 # start wavelength in nm - min 1450 nm
wth_f = 1650 # stop wavelength in nm - max 1650 nm
...
osc.setStartTime(scan_time/2) # scan_time/2
...
def Acquisition(data_crop=False, plot=True, save=False):
    ...
    if plot:
        fig, axspec = plt.subplots(4, sharex=True)
        axspec[0].plot(df['time'].values, df['trigger_laser'].values, ".")
        axspec[1].plot(df['time'].values, df['cav'].values,".")
        axspec[2].plot(df['time'].values, df['acetylene'].values,".")
        axspec[3].plot(df['time'].values, df['mzi'].values,".")
        axspec[3].set_xlabel("Time (s)")
        axspec[0].set_ylabel("Voltage (V)")
        axspec[1].set_ylabel("Voltage (V)")
        axspec[2].set_ylabel("Voltage (V)")
        axspec[3].set_ylabel("Voltage (V)")
        axspec[0].set_title("Laser Trigger")
        axspec[1].set_title("cav")
        axspec[2].set_title("Acetylene")
        axspec[3].set_title("mzi")
        fig.tight_layout()
    ...
    if save:
        ...
        filename = '04NOV21_chip_3 degenerated coupled rings_wg-ring gap 800 nm_ring-ring gap 550 nm_1650 TE'
